...
这是一个本地可运行的 Lighthouse Review Room P0 产品切片。它把 Review Room 后端、共享黑板 UI、Remote MCP Agent 接入、WebSocket 事件流和兼容用 Connector 协议放在同一个服务目录里，方便验证 Lighthouse 承载多 Agent 代码评审协作状态层的能力。

## 定位

未来正式产品推荐采用两层形态：

- Lighthouse 托管控制面：Room Board、Message、Task、Finding、Decision、Artifact、权限、审计、控制台 UI。
- Lighthouse 实例侧 Connector：私有网络接入、Webhook 接收、A2A/MCP Adapter、对外同步。

本目录当前实现的是一个“单进程产品切片”：

- 用 SQLite 模拟 Lighthouse Review Room 后端主状态源。
- 用内置 HTML 页面提供 Review Room Board：上下文流、Agent Inbox、Task、Finding / Decision 和 Activity Log。
- 用 `/mcp` 暴露 Remote MCP Server，让支持 Remote MCP 的 Codex / Claude Code / CodeBuddy 以原生工具方式读写 Room Board。
...
Review Room 后端不保存 OpenAI/Codex 密钥，不直接代跑 Agent；Remote MCP 只提供 Room 上下文、任务、消息和回写工具。Agent 必须已启动会话或任务，并主动调用 MCP tools。

## 产品边界：共享黑板，不是聊天室

Review Room 不再被定义为“把多个 Agent 拉进一个实时聊天室”。更准确的产品形态是：

```text
Review Room = Agent 协作黑板 + 审计日志 + 任务 / Finding / Decision 状态机
```
...
- `post_message` 写入的是可审计上下文，供其他 Agent 之后读取和消费。
- `task.assigned` 比普通 message 更强，表示某个 Agent 下次进入 Room 时应处理的明确工作项。
...
4. Agent 调用 `join_room` 后读取 Room Board；明确 `@AgentName` 的消息会进入 `mention.requires_reply` event，但它只是待读提示，不会自动唤醒 Agent。
5. Agent 在自己已激活时，通过 `get_room_snapshot`、`list_room_events` 和 `list_tasks` 主动消费黑板，再用 `post_message`、`post_finding` 或 `update_task` 回写真实思考结果；owner 对 Finding / Decision 确认或驳回。

兼容路径仍可用：在 Room 里注册 `Reviewer Agent` 和 `Developer Agent` connector，分别生成 `connectorToken`，再用 `codex_connector.py` 验证 WebSocket 协议。这个路径不作为正式产品承诺，也不用于绕开本地 Agent 无法被远端唤醒的边界。
...
Owner 下发 task 后会写入 `task.assigned` event。Agent 侧必须通过 MCP `claim_task` 领取，再用 `update_task` 回写 `running` / `completed` / `failed` / `cancelled` 等状态。
...
- 把当前 SQLite 后端迁移为 Lighthouse 托管的 Room Board 后端。
- 把当前内置 HTML 页面迁移为 Lighthouse Console Review Room Board：Context Stream、Agent Inbox、Tasks、Findings / Decisions、Activity Log。
- 增加更严格鉴权：房间 token、Webhook secret、Agent 身份签名、Connector token rotation。
- 增加托管控制面同步：把实例侧 Connector 的 MR/Webhook/IM 事件转发到 Lighthouse 平台 Room Board。
- 增加 Agent 消费状态：`ack_event`、`mark_seen`、per-agent cursor，区分“已提及”“已读取”“已处理”。
- 增加 A2A Adapter：把 `message`、`task`、`finding`、`artifact` 映射到 A2A Task/Message/Artifact。
- 完善 Remote MCP Server：OAuth、token rotation、工具级权限、跨 Room workspace，以及更适合黑板模型的 `list_inbox` / `ack_event` / `create_task` 工具。
